...
4. The Worker starts one asynchronous Browser Run `/crawl` job restricted to the selected sitemap URLs, using fast origin HTML without a browser instance, same-origin traversal, depth five, fresh origin content, and `search` plus `ai-input` crawl purposes.
5. `source_runs` persists the upstream job ID, phase, result cursor, start time, upstream finished/total progress, counters, and initial-crawl flag. Each Queue delivery renews the source lease and either polls status after 10 seconds or advances one result page before requeueing.
6. A crawl still running after two hours is cancelled and failed. HTTP 429 honours `Retry-After`, transient 5xx failures retry, and 401/403 responses fail as non-retryable operator configuration errors.
