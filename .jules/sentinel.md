...
## 2026-03-28 - [CORS Bypass via Extension Origin Allowlist]
**Vulnerability:** The `applyCorsHeaders` handler was granting `Access-Control-Allow-Origin` unconditionally to any origin using extension protocols (`chrome-extension:`, `moz-extension:`, `safari-web-extension:`).
**Learning:** Browser extensions have access to sensitive permissions and their origins take the form `protocol://<extension-id>`. Granting CORS dynamically to *all* extension origins exposes authenticated APIs to malicious extensions installed by the user. While extension protocols prevent some typical web-based XSS attacks, allowing any extension is an overly permissive and dangerous wildcard behavior.
**Prevention:** Always validate the hostname of the extension origin (which corresponds to the specific extension ID) against a strict, explicit allowlist of authorized extensions (e.g., via an `ALLOWED_EXTENSION_IDS` environment variable).
