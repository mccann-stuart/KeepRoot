...
## 2026-03-28 - [SSRF via Unvalidated Redirects and Internal IPs]
**Vulnerability:** A Server-Side Request Forgery (SSRF) vulnerability existed in `saveItemFromUrl` where user-supplied URLs were fetched with `redirect: 'follow'` and without validating whether the initial URL or any subsequent redirects pointed to internal/private IP addresses or hostnames.
**Learning:** Fetch implementations that automatically follow redirects can be abused to bypass initial SSRF checks. An attacker can provide a safe URL (e.g., `http://attacker.com`) that passes validation, but the server at that URL redirects to a private IP (e.g., `http://169.254.169.254/latest/meta-data/`), causing the backend to fetch internal metadata or interact with internal services.
**Prevention:** To prevent SSRF bypasses via redirects, use `redirect: 'manual'` when fetching untrusted URLs. Implement a custom loop to manually follow `Location` headers, validating each redirected URL using a strict IP/hostname allowlist (rejecting local hostnames and private IPv4/IPv6 ranges) before fetching the next URL.
