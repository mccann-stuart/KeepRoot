...
## 2026-04-04 - [D1 Batching vs Maintainability]
**Learning:** Removing shared database helper functions and replacing them with duplicated inline SQL purely to force multiple queries into a single `D1Database.batch()` call is a destructive anti-pattern. While it saves network roundtrips, it creates a massive maintainability burden and risks bypassing important logic (like data mapping or authorization checks) embedded within the helper functions.
**Action:** When optimizing database reads, do not inline SQL at the expense of code reuse. If batching is critical, refactor helper functions to return prepared statements rather than executing them, allowing the caller to construct the batch safely without duplicating SQL strings.
