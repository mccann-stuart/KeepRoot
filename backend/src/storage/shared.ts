...
export async function validateSafeUrl(url: string): Promise<boolean> {
	try {
		...
		// Prevent SSRF bypasses via URL parser discrepancies by rejecting URLs with embedded credentials.
		if (parsedUrl.username !== '' || parsedUrl.password !== '' || url.includes('@' + parsedUrl.host)) {
			return false;
		}
