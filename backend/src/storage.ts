...
async function getTableColumnNames(env: StorageEnv, tableName: string): Promise<Set<string>> {
	const result = await env.KEEPROOT_DB.prepare(`PRAGMA table_info(${tableName})`).all<D1ColumnInfo>();
	return new Set(result.results.map((column) => column.name));
}

async function runSchemaStatement(env: StorageEnv, sql: string): Promise<void> {
	try {
