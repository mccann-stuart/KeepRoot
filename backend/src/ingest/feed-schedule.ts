...
export function clampPollIntervalMinutes(value: number | null | undefined): number {
...
export function calculateAdaptivePollIntervalMinutes(
	...
	if (postsPerHour < 0.01) {
		return MAX_POLL_INTERVAL_MINUTES;
	}
...
export function calculateNextPollAt(now: Date, intervalMinutes: number): string {
	return new Date(
		now.getTime() + clampPollIntervalMinutes(intervalMinutes) * MILLISECONDS_PER_MINUTE,
	).toISOString();
}
